        ...
        <div className="flex items-center">
          <Link href="/" className="flex items-center gap-3 group">
            <div className={cn(
              ...
              isTransparent ? "bg-white text-slate-950 group-hover:scale-110" : "bg-accent text-accent-foreground group-hover:rotate-12"
            )}>
              SC
            </div>
            <span className={cn(
        ...
        <nav className="hidden lg:flex items-center absolute left-1/2 -translate-x-1/2">
          <div className={cn(
            ...
            isTransparent 
              ...
              : "bg-slate-800/50 backdrop-blur-lg border-white/5"
          )}>
            {navigation.map((item) => (
              <Link
                key={item.href}
        ...
        <div className="flex items-center gap-4">
          <Button asChild variant="outline" className={cn(
            ...
            isTransparent 
              ...
              : "border-accent/30 bg-accent/5 text-accent hover:bg-accent hover:text-accent-foreground"
          )}>
          ...
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            ...
            <SheetContent side="right" className="w-[300px] sm:w-[400px] bg-slate-950 border-slate-900 text-white p-8">
              <div className="flex items-center gap-3 mb-12">
                 <div className="bg-accent text-accent-foreground p-2 rounded-none font-black text-xs">SC</div>
              ...
              <nav className="flex flex-col gap-6">
                {navigation.map((item) => (
                  <Link
                    ...
                    className={cn(
                      ...
                      pathname === item.href ? "text-accent" : "text-white/60 hover:text-white"
                    )}
                  >
                    {item.name}
                  </Link>
                ...
                <div className="mt-12">
                  <Button asChild className="w-full h-14 rounded-full bg-accent text-accent-foreground hover:opacity-90 font-bold text-sm tracking-widest uppercase">
